...
**Scope notes (added after initial build)**:
- Structured output uses `client.messages.parse()` + `output_config.format` (Zod schema) rather than tool-use — the recommended approach for "shape this JSON" tasks; Claude Opus 5 with adaptive thinking, per the project's model defaults. `AIInvocationPurpose` also covers `PREP_DIGEST` (the Prep Card's transient "AI Digest" line) — a small second Claude call, plain text not structured, logged the same way; `AIInvocationLog.interactionId` is nullable because that call happens before any `Interaction` exists yet (`memberId` is always set instead, so every invocation stays traceable).
- `AuditLogEntry` itself (the `actorType=AI` audit trail the Continuity Engine narrative mentions) is deferred to Sprint 6, where Lead actions actually exercise it — Sprint 4's own schema-extension bullet above only calls for `AIInvocationLog`.
- **Verified without a live Anthropic API key**: `ANTHROPIC_API_KEY` is still unset in this environment. The Claude-calling paths (`generateCaptureDraft`, `generatePrepDigest`) are code-complete and reviewed but not exercised live; both degrade gracefully (`LLMNotConfiguredError` → a clear message on the Prep Card / Capture screen, never a crash) — verified by triggering that exact path. Everything else — manual typing, real local Whisper transcription, consent gating, draft editing, action-item add/remove, and the approve→`ContextLedgerEntry` transition — was verified against a manually-inserted draft standing in for a completed Claude call, so the human-approval boundary itself is fully verified even though the draft-generation call isn't.
- **Two real bugs caught during verification**:
  1. **Stale prefetch cache on every Capture mutation** — same root cause as Sprint 1's post-login bug: a successful Server Action that redirects back to the *same* URL a browser prefetch may already hold a stale RSC payload for can serve that stale snapshot instead of the fresh one the action just produced. Six actions on the Capture page all redirect to the same base URL, so this wasn't a one-off — fixed with a small shared `freshCapturePath()` helper that appends a cache-busting `?_r=<timestamp>` on every success redirect, cheaper here than a dedicated resolver route per action.
  2. **Whisper's `tiny` model mis-detected the language** on a short English test clip and transliterated it into Hebrew script entirely. Fixed by pinning `language=en` on the transcription request rather than relying on auto-detect, since the product is English-only for now (see "UI Language & Hebrew Readiness"); worth revisiting if the product itself goes multi-language.
- Local Whisper runs via `onerahmet/openai-whisper-asr-webservice` (`ASR_ENGINE=faster_whisper`, `ASR_MODEL=tiny`) — fast on CPU for local dev, real speech-to-text confirmed end-to-end with an actual recorded clip (macOS `say` → real audio file → uploaded → transcribed → saved), not just a wiring check. `tiny` trades accuracy for speed (e.g. "walking" heard as "working" in testing); a larger model is a one-line `ASR_MODEL` change when accuracy matters more than local dev speed.
...
**Scope notes (added after initial build)**:
- `ReviewQueueItem.flagType` (`SAFETY_KEYWORD`/`MISSED_FOLLOW_UP`/`BOUNDARY_BREACH`/`LLM_FLAGGED`) and `flagSource` (`RULE_ENGINE`/`LLM`) are separate fields — the first three flag types always pair with `RULE_ENGINE`, the last always with `LLM`, matching the Lead's review-queue mockup ("SAFETY_KEYWORD · via RULE_ENGINE").
- `missedFollowThrough.ts` sweeps the *whole* Member's open items on every approval, not just what the just-approved capture mentioned — an overdue commitment from three captures ago still surfaces the next time anything for that Member is approved. It dedupes via `reviewQueueItems: { none: ... } }` so the same overdue item is never re-flagged.
- `boundaryBreach.ts` covers the product's core anti-disintermediation principle (see the top-level product framing) as a hard deterministic rule, not something left to the LLM's judgment — phone numbers, emails, and phrases like "outside the app" or "whatsapp".
- **Real bug caught during verification**: `checkBoundaryBreach` can return multiple matches for one message (e.g. a phone number *and* "outside the app" in the same summary). The first version created one `ReviewQueueItem` per match, which would give a Lead two near-duplicate open items to triage for what's really one concerning message. Fixed to consolidate all boundary-breach matches for an interaction into a single flag with a combined rationale. (Safety keywords deliberately stay one-flag-per-match, since each configured keyword carries its own independently meaningful severity.)
- The LLM nuance pass (`generateSurfaceAnalysis`, `AIInvocationPurpose.SURFACE_ANALYSIS`) degrades the same way as every other Claude call in this codebase: with no `ANTHROPIC_API_KEY`, it's skipped silently and the rule-based flags still stand — verified explicitly (no `LLM_FLAGGED` item, no `SURFACE_ANALYSIS` log entry, no crash).
- Verified two ways: a direct script exercising `runSurfaceAnalysis()` against hand-built fixtures (15 checks — flag creation, correct `flagSource`/severity, the missed-follow-up dedupe, graceful LLM skip), and a full browser run through the *real* `approveCaptureAction` (login → Capture → Approve) to confirm the wiring fires through actual Server Action dispatch, not just a direct function call.
- Found the same stale-redirect-cache bug (Sprint 1, Sprint 4) a fourth time, now in `admin.ts`'s `addSafetyKeywordAction`/`removeSafetyKeywordAction` — both redirected to a bare `/admin/settings` on success. Applied the same cache-busting-redirect fix. At this point the pattern is clearly systemic to any Server Action that redirects to a URL a client prefetch might already hold stale — worth defaulting *every* new success-redirect in this codebase to the cache-busted form rather than rediscovering this bug sprint by sprint.
